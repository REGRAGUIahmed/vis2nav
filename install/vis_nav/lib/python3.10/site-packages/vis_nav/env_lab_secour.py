...
class DepthImage_subscriber(Node):
    ...
    def listener_callback(self, msg):
        try:
            ...
            # Assuming the depth image is single-channel, no need to convert to grayscale
            # gray_image = cv2.cvtColor(cv_image_normalized, cv2.COLOR_BGR2GRAY)
            global last_image
            #last_image = cv_image_normalized[:440, :600]  # Crop to (440, 640)
            last_image = cv_image_normalized[80:400, 140:500]  # Crop to (440, 640)
            
            
            #last_image = cv_image_normalized  # Crop to (440, 640)
            
            # If you need to use a global variable, you can set it here
            # global last_image
